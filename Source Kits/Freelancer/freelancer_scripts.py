...
####### NOTES: returns only first 3 valid troops. Max valid level is 30
  ...
  [
    ...
    (try_end),
    (assign, ":index", 0),## index for different registers
    (try_for_range, ":cur_troop", regulars_begin, regulars_end),
        (troop_get_slot, ":cur_culture", ":cur_troop", slot_troop_culture),
        (eq, ":cur_culture", ":freelancer_culture"),## if troop culture = freelancer culture
        ...
        (try_begin),
            ...
            (ge, ":player_level", ":cur_level"),
            (try_begin),
                (eq, ":index", 1),
                (gt, ":cur_troop", reg51),
                (assign, reg51, ":cur_troop"),
                (val_add, ":index", 1),
                (try_begin),
                    (gt, ":index", 3),
                    (assign, ":index", 1),
