  ...
  [
    ...
    (try_for_range, ":cur_troop", regulars_begin, regulars_end),
        ...
        (try_begin),
            ...
            # (display_message, "@called script_freelancer_get_highest_values cavalry for {s20}"),
        (else_try),
            (eq, ":division", grc_archers),
            (neg|troop_is_guarantee_horse, ":cur_troop"),
            (troop_is_guarantee_ranged, ":cur_troop"),
            (call_script, "script_freelancer_get_highest_values", ":cur_troop"),
            # (display_message, "@called script_freelancer_get_highest_values archers for {s20}"),
        (else_try),
            (eq, ":division", grc_infantry),
            (neg|troop_is_guarantee_horse, ":cur_troop"),
            (neg|troop_is_guarantee_ranged, ":cur_troop"),                
            (call_script, "script_freelancer_get_highest_values", ":cur_troop"),
            # (display_message, "@called script_freelancer_get_highest_values infantry for {s20}"),
        (try_end),
    (try_end),
  ]),    
...
####### NOTES: the lowest value is lost
  ...
  [
    ...
    (try_begin),
        ...
        (assign, ":aux1", reg51),
        (assign, reg51, ":number"),
        # (display_message, "@1299"),
    (else_try),
        (gt, ":number", reg52),
        (assign, ":aux1", reg52),
        (assign, reg52, ":number"),
        # (display_message, "@1304"),
    (else_try),
        (gt, ":number", reg53),
        (assign, ":aux1", reg53),
        (assign, reg53, ":number"),
        # (display_message, "@1309"),
    (try_end),
    (try_begin),
        (gt, ":aux1", reg51),
        (assign, ":aux2", reg51),
        (assign, reg51, ":aux1"),
        # (display_message, "@1315"),
    (else_try),
        (gt, ":aux1", reg52),
        (assign, ":aux2", reg52),
        (assign, reg52, ":aux1"),
        # (display_message, "@1320"),
    (else_try),
        (gt, ":aux1", reg53),
        (assign, ":aux2", reg53),
        (assign, reg53, ":aux1"),
        # (display_message, "@1325"),
    (try_end),
    (try_begin),
        (gt, ":aux2", reg51),
        (assign, reg51, ":aux2"),
        # (display_message, "@1330"),
    (else_try),
        (gt, ":aux2", reg52),
        (assign, reg52, ":aux2"),
        # (display_message, "@1334"),
    (else_try),
        (gt, ":aux2", reg53),
        (assign, reg53, ":aux2"),
        # (display_message, "@1338"),
    (try_end),
   ]),    
