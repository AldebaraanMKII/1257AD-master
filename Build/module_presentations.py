  ...
  ("game_start",prsntf_read_only,0,[
    (ti_on_presentation_load,
      [
        ...
        (create_text_overlay, "$g_presentation_credits_obj_1", "@Anno Domini 1257, Enhanced Edition", tf_double_space),
    ...
    (ti_on_presentation_load,
     [(set_fixed_point_multiplier, 1000),
      ...
      (else_try),
        ...
        (assign, ":cur_troop", "trp_mamluk_cavalry_3"),
      (else_try),
...
##        (overlay_set_text, "$g_multiplayer_team_2_score_display_overlay", "str_s0_dd_reg0"),
  ...
  # ("banner_selection",0,mesh_load_window,[
      # (ti_on_presentation_load,
       ...
       ################# # (create_button_overlay, "$g_presentation_obj_banner_selection_2", "@Previous Page", tf_center_justify),
       ################# # (position_set_x, pos1, 100),
       ################# # (position_set_y, pos1, 50),
       ################# # (overlay_set_position, "$g_presentation_obj_banner_selection_2", pos1),
      ...
      # (ti_on_presentation_event_state_change,
       ...
       ############## # (else_try),
       ##############   # (eq, ":object", "$g_presentation_obj_banner_selection_2"),
...
############################## NEW v2.1 - New banner presentation (all banners in a single page).
("banner_selection",0,mesh_load_window,[
      (ti_on_presentation_load,
       [(set_fixed_point_multiplier, 1000),
        (str_store_string, s1, "str_banner_selection_text"),
      ...
      [
        ...
        (else_try),
          ...
          (try_for_range_backwards, ":loop_var", "trp_kingdom_heroes_including_player_begin", active_npcs_end),
	        (troop_slot_eq, ":loop_var", slot_troop_is_alive, 1),  ## he's alive/active
            (ge, ":cur_index", 0),
...
####### NEW v2.9-KOMKE START-            
      ...
      #   # (store_sub, ":var0", "fac_kingdoms_end", 1),
      #   (store_sub, ":var0", "fac_enhanced_kingdom_1", 1), #### NEW v2.1 - bugfix, otherwise empty factions would appear
      #   (assign, "$g_start_faction", ":var0"),
      #   (try_begin),
      #     (eq, "$kaos_monarch_var", 1),
      #     (store_sub, ":var0", "$kaos_kings_kingdom", 1),
      #   (else_try),
      #     (eq, "$kaos_heir", 1),
      #     (store_sub, ":var0", "$kaos_kings_prince", 1),
      #   (else_try),
      #     (eq, "$start_as_vassal_gvar1", 1),
      #     (store_sub, ":var0", "$kaos_kings_vassal", 1),
      #   (try_end),
      # (else_try),
      #   # (ge, "$g_start_faction", "fac_kingdoms_end"),
      #   (ge, "$g_start_faction", "fac_enhanced_kingdom_1"), #### NEW v1.0 - bugfix, otherwise empty factions would appear
      #   (assign, "$g_start_faction", "fac_kingdom_1"),
      #   (try_begin),
      #     (eq, "$kaos_monarch_var", 1),
      #     (assign, "$kaos_kings_kingdom", 1),
      ...
      #   (try_end),
      # (try_end),
      (try_begin),  ##initialize g_start_faction to kingdom1 if not in range of kingdoms
        (this_or_next|lt, "$g_start_faction", "fac_kingdom_1"),
        (gt, "$g_start_faction", "fac_kingdom_42"),
        (assign, "$g_start_faction", "fac_kingdom_1"),
        (try_begin),## if initializing initialize kaos variables too
          (eq, "$kaos_monarch_var", 1),
          (assign, "$kaos_kings_kingdom", 1),
        (else_try),
          (eq, "$kaos_heir", 1),
        ...
        (try_end),
      (try_end),          
      (store_sub, ":var1", "$g_start_faction", "fac_kingdom_1"),
      (set_fixed_point_multiplier, 1000),
      ...
      (store_add, "$g_presentation_banner_start", "$g_presentation_obj_banner_selection_1", 1),
      (store_add, ":var2", "mesh_pic_arms_teutonic", ":var1"),
      (create_mesh_overlay, reg1, ":var2"),
      (position_set_x, pos1, 500),
      (position_set_y, pos1, 350),
      (overlay_set_position, reg1, pos1),
      (presentation_set_duration, 999999),
    ...
    [
      ...
      (try_begin),
        (eq, ":var0", "$g_presentation_obj_faction_selection_2"),##button next
        ###(play_sound, "snd_choose_faction_menu"),
        (val_add, "$g_start_faction", 1),
        (try_begin),        
          (gt, "$g_start_faction", "fac_kingdom_42"),##KOMKE if loop > last normal kingdom loop = first normal kingdom
          (assign, "$g_start_faction", "fac_kingdom_1"),
        (try_end),  
        (try_begin),
          (eq, "$kaos_monarch_var", 1),
      ...
      (else_try),
        (eq, ":var0", "$g_presentation_obj_faction_selection_3"),##button previous
...
####### NEW v2.9-KOMKE END-                           
      (else_try),
        (eq, ":var0", "$g_presentation_obj_faction_selection_4"),##button accept
        ###(play_sound, "snd_choose_faction_menu"),
        (presentation_set_duration, 0),
        (try_begin),
          (eq, "$kaos_monarch_var", 1),
     ...
     [
      ...
      (try_begin), #set values here
        ...
        (try_begin), #populate damage type slider
          ...
          (this_or_next|is_between, ":slot_value", itp_type_one_handed_wpn, itp_type_shield),
          (eq, ":slot_value", itp_type_thrown), #throwing axe vs jaridss vs rocks
    ...
    (ti_on_presentation_event_state_change,
     [
        ...
        (try_begin),
          ...
          (call_script, "script_update_wpn_slot_itp", slot_upgrade_wpn_0, ":value"),
        (else_try),
          (eq, ":object", "$g_presentation_obj_2"),
          (call_script, "script_update_wpn_slot_itp", slot_upgrade_wpn_1, ":value"),
        (else_try),
          (eq, ":object", "$g_presentation_obj_3"),
          (call_script, "script_update_wpn_slot_itp", slot_upgrade_wpn_2, ":value"),
        (else_try),
          (eq, ":object", "$g_presentation_obj_4"),
          (call_script, "script_update_wpn_slot_itp", slot_upgrade_wpn_3, ":value"),
        (else_try),
          (eq, ":object", "$g_presentation_obj_10"),
          (presentation_set_duration, 0),
        (else_try),
          (eq, ":object", "$g_presentation_obj_11"),
          (call_script, "script_dplmc_copy_upgrade_to_all_heroes", "$temp", wpn_setting_1),
        (else_try),
        ...
        (else_try),
          ...
          (call_script, "script_dplmc_copy_upgrade_to_all_heroes", "$temp", armor_setting),
        (else_try),
          (eq, ":object", "$g_presentation_obj_16"),
          (call_script, "script_dplmc_copy_upgrade_to_all_heroes", "$temp", horse_setting),
        (else_try),
          (eq, ":object", "$g_presentation_obj_sliders_1"), #damage type
          (is_between, "$temp_2", slot_upgrade_wpn_0, slot_upgrade_wpn_3 + 1), #valid range
...
####### NEW v2.9-KOMKE START-        
        ## default and random NEW v2.9-KOMKE Diplomatic button and Customize button
...
####### NEW v2.9-KOMKE END-                     
        (else_try),
          (eq, ":object", "$g_presentation_obj_12"),##Randomize colors button
...
########################################## ROW 1 COLUMN 2
        ...
        (position_set_y, pos1, 686),   #####TAKE 14 OUT OF @Travel speed y
        (position_set_x, pos1, 390),
        (overlay_set_position, "$g_presentation_obj_10", pos1),
